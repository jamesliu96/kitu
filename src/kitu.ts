...
export class Kitu {
  static KituError = class extends Error {
    constructor(type = 'KituError', message?: string, options?: ErrorOptions) {
  ...
  #mousedown = ({ offsetX: x, offsetY: y }: MouseEvent) => {
    ...
    for (const thing of this.things) {
      ...
      if (this.#activePoint) break;
    }
  };
  #mousemove = ({ movementX: dx, movementY: dy }: MouseEvent) => {
    if (this.#activePoint)
      this.#activePoint.rotatePos = this.#activePoint.rotatePos.plus(
        new Vec(dx, dy)
      );
  };
  #mouseup = () => {
  ...
  #dblclick = ({ offsetX: x, offsetY: y }: MouseEvent) => {
  ...
  #contextmenu = (e: MouseEvent) => {
    ...
    if (this.#selectPoint)
      this.#selectPoint.forces.add(
        new Vec(x, y).minus(this.#selectPoint.offsetPos)
      );
  };
  #mouseEnabled = false;
  get mouseEnabled() {
  ...
  #frame() {
    ...
    for (const thing of this.things) {
      ...
      drawPoint(
        ...
        'transparent',
        undefined,
        `${((180 / Math.PI) * thing.rotation).toFixed()}°`,
        'green'
      );
    }
  }
